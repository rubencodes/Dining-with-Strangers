<?php session_start(); include_once( 'root.php'); include_once( 'domain/Encryption.php'); include_once( 'database/dbUser.php'); include_once( 'database/dbStranger.php'); include_once( 'database/dbDate.php'); include_once( 'database/dbPartner.php'); $converter=n ew Encryption; $UID=$ converter->decode($_SESSION["BowdoinDwSuserID"]); $user = retrieve_dbUser($UID); if(!$user) $user = retrieve_dbStranger($UID); ?>
<!doctype html>
<html>
...
<head>
	...
	<? if($_GET[ 'offmail']=="t" ) $note="<div id=g-notification>You've been removed from our email list.</div>" ; if($_GET[ 'offmail']=="f" ) $note="<div id=b-notification>Something's gone wrong while removing you from our list. Please try again later.</div>" ; if(!empty($_GET[ 'id']) && !empty($_GET[ 'email'])) { if($u=r etrieve_byEmail_dbStranger($_GET[ 'email'])) { if($u->get_id() == $_GET['id'] && $u->get_active() == "f") { $u->set_active("t"); $_SESSION['firstsignin'] = "true"; if(update_dbStranger($u)) $note = "
	<div id=g-notification>Your email is confirmed! You may now sign in.</div>"; } else $note = "
	<div id=b-notification>This account has already been confirmed.</div>"; } } elseif($user instanceof User && $user->get_active() == 't') { $_SESSION['firstsignin'] = "true"; $user->set_active('true'); update_dbUser($user); } if ($user) echo "
	<style>
		body {
			background: #EEE;
		}
	</style>"; if (isset($_SESSION['firstsignin'])) echo "
...
<body>
	...
	<table id=navbar class=box>
		<tr>
			<td id=DwS><a href=/>
				<img src=images/32X.png width=30 height=30 />Dining with Strangers</a>
			</td>
			<? if (!$user) echo "
...
<p id=updateUser class='icon ion-person toolbar-link'></p>"; if($user->get_access() > 1) echo "
			<p id=submitPartner class='icon ion-fork toolbar-link'></p>"; echo"
			</td>
			<td style=float:right;>
				<a href=handlers/handleLogout.php>
					<button id=navbar-logout class='btn btn-secondary btn-small' type=button>Logout</button>
				</a>"; } ?>
			</td>
		</tr>
	</table>