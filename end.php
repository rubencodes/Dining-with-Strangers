<footer class=box>
	<a href="ToC.php">Terms of Service &amp; Privacy Policy</a>
	<br>
	<a href="FAQ.php">FAQ</a>
	<br>
	<a href="CommentCard.php">Leave Us a Comment Card</a>
	<br>Dining with Strangers &copy;
	<?php echo date( 'Y'); ?>
	<br>
...
<script>
	$(document).ready(function () {
		...
		$('#mydates').on('click', "button[id^='date-']", function () {
			...
			posting.done(function (data) {
				$('body').append(data);
				setTimeout(function () {
					$('[id$=notification]').fadeOut(500, function () {
						$(this).remove();
					});
				}, 3000);
				if ($('#g-notification').length > 0) $('form' + id).slideUp(function () {
					$(this).remove()
				});
			});
		});
			...
			$('#options').change(function () {
				...
				if (option === "location") {
					...
					if ($user) {
						...
						foreach($partners as $partner) echo "$('#filter1').append(\"<option value='".$partner - > get_id().
						"'>".stripslashes(urldecode($partner - > get_name())).
						" ".urldecode($partner - > get_promos()).
						"</option>\");";
					} ?>
				} else if (option === "time") {
					var now = <? echo "'".date('Y-m-d').
					"'"; ?> ;
					var max = <? echo "'".date('Y-m-d', strtotime(date('Y-m-d').
						' + 6 days')).
					"'"; ?> ;
		...
		$(window).on("resize", function () {
			$('#map').prop('width', $('#map').parent().css('width'));
		});
		var pageCount = 0;
	...
	function clearOverlay() {
		...
		$('#shade,#frame,#close').fadeOut(500, function () {
			$('#shade,#frame,#close').remove();
		});
	}
	var released = true;
